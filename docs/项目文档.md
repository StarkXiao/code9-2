...
| 内容版本 | content_version | 条目内容每次变更 +1 的版本号，协同编辑三方合并的乐观并发令牌 |
| 编辑冲突 | edit conflict | 多人同时把同一字段改成不同值时，系统为双方各留一份、等待逐项确认的记录 |
...
#### 5.3.1 协同编辑：字段级自动合并与冲突确认

多人可能同时编辑同一条目（例如作者与审核员各自打开编辑页）。系统不允许"后保存的覆盖先保存的"这种静默丢改。

**处理**：

1. 条目维护 `content_version`，每次内容变更 +1；每次变更落库时写一份编辑快照（`spot_edit_snapshots`）。
2. 编辑页打开时带上当前版本号，保存时回传 `baseVersion`。
3. `baseVersion` 与当前版本一致 → 直接保存。
4. `baseVersion` 落后 → 以该版本快照为基准做**三方合并**（base / 当前 / 我的提交），逐字段比较：
   - 我没改的字段 → 保留他人改动；
   - 对方没改的字段 → 采用我的改动（属性按 key 合并；经纬度、模糊设置、图片列表各自视为一个整体字段）；
   - 双方都改了且不一样 → **冲突**：无冲突字段照常自动合并落库，冲突字段保留当前值，双方版本各存一份（`spot_edit_conflicts`），接口返回 409 `EDIT_CONFLICT`。
5. 编辑页展示冲突面板：每个冲突字段并排展示"当前内容"与"对方的修改"，编辑者**逐项确认取舍**（保留当前 / 采用对方），每项确认即落库对应字段。
6. 冲突登记后该字段若又有新改动，旧冲突自动作废（`superseded`），页面提示刷新，避免旧提案覆盖更新内容。

**反馈**：自动合并成功时页面提示"已自动合并他人对其他字段的修改"并刷新表单；冲突来自他人条目时，作者收到 `edit_conflict` 站内通知，点击直达编辑页处理。

**闭环出口**：冲突只有 `resolved` / `superseded` 两个出口，不会滞留；合并与取舍操作均写审计日志。
...
| `edit_conflict` | 自己的条目出现协同编辑冲突 | 条目编辑页（冲突面板） |
...
-- ============ 协同编辑：冲突字段双方各留一份 ============
CREATE TYPE edit_conflict_status AS ENUM ('open','resolved','superseded');
CREATE TABLE spot_edit_conflicts (
  id             BIGSERIAL PRIMARY KEY,
  uuid           UUID NOT NULL UNIQUE DEFAULT gen_random_uuid(),
  spot_id        BIGINT NOT NULL REFERENCES spots(id) ON DELETE CASCADE,
  field          VARCHAR(64) NOT NULL,        -- title / attributes.<key> / location / fuzz / media ...
  base_value     JSONB,
  current_value  JSONB,
  proposed_value JSONB,
  proposed_by    BIGINT NOT NULL REFERENCES users(id),
  status         edit_conflict_status NOT NULL DEFAULT 'open',
  resolution     VARCHAR(16),                 -- current / proposed
...
| 409 | `EDIT_CONFLICT` / `EDIT_CONFLICT_STALE` | 协同编辑冲突待逐项确认 / 冲突已过期作废 |
...
| PATCH | `/spots/:uuid` | 作者 | 更新草稿或被要求修改的条目；携带 `baseVersion` 时启用三方合并（见 5.3.1） |
...
| GET | `/spots/:uuid/edit-conflicts` | 作者/moderator/冲突当事人 | 协同编辑冲突列表（`?status=open`） |
| POST | `/spots/:uuid/edit-conflicts/:conflictUuid/resolve` | 作者/moderator/冲突当事人 | 逐项确认取舍（`{ choice: "current" \| "proposed" }`） |
...
| A16 | 协同编辑按字段自动合并，冲突双方各留一份 | 两人基于同一版本各改不同字段 → 两份修改都保留；同改一个字段 → 返回 409 `EDIT_CONFLICT`，编辑页逐项确认取舍后落库，另一份修改不被静默覆盖 |
...
- [ ] 修订历史可查看并对比差异
- [ ] 作者可删除自己的条目，发布中的转为归档
- [ ] 两人基于同一版本各改不同字段，保存后两份修改都保留（自动合并）
- [ ] 两人同改一个字段，后保存者收到 409 与冲突清单，编辑页出现冲突面板
- [ ] 冲突可逐项"保留当前 / 采用对方"，确认后对应字段落库、冲突关闭
- [ ] 冲突登记后字段又有新改动时，旧冲突作废且不会覆盖新内容
