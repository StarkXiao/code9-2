...
**并发编辑合并**：同一条目可能被作者与审核员同时编辑（例如作者补充描述时，审核员正在修正标题）。保存时按字段自动合并，冲突字段保留两份由编辑者逐项确认：

1. 编辑页加载时记录基线：条目的 `updatedAt`（版本标记）与表单快照（base）。
2. 保存时客户端带回 `baseUpdatedAt` 与 `base`；服务端发现条目在此期间被他人保存过，即做**三路合并**（base / 服务端当前值 / 本次提交）：
   - 只有一方改过的字段 → 自动采用改动方，保存成功并告知合并了哪些字段；
   - 双方改了同一字段且结果不同 → 返回 `409 EDIT_CONFLICT`，冲突字段的三份值（base/对方/我方）随响应一并返回；
   - 位置（经纬度）与模糊设置（开关+半径）各自作为整体字段合并，避免拼出坏坐标；结构化属性按属性 key 逐个合并；图片列表按集合合并（双方增删都生效），合并后超过 6 张上限时整个列表作为冲突项二选一。
3. 编辑者在页面上逐项选择"我的改动 / 对方的改动"，确认后带 `resolveConflicts: true` 重新提交，冲突字段以提交值为准；确认期间对方再次保存的，重新计算冲突并刷新面板。
4. 不带 `baseUpdatedAt` 的调用保持原有直接保存行为（兼容旧客户端与脚本）。
...
| 409 | `EDIT_CONFLICT` | 条目编辑冲突，需逐项确认取舍 |
...
**PATCH /spots/:uuid 的并发合并约定**

除原有表单字段外，请求体可携带三个合并控制字段（均可选，缺省即原来的直接保存）：

```json
{
  "title": "新的标题",
  "baseUpdatedAt": "2026-09-20T08:00:00.000Z",
  "base": { "title": "打开页面时的标题", "description": "...", "attributes": {} },
  "resolveConflicts": false
}
```

- `baseUpdatedAt` 与服务端当前 `updatedAt` 一致 → 无并发，直接保存；
- 不一致 → 以 `base` 为基线做字段级三路合并：无冲突则保存合并结果，响应带 `merge.autoMergedFields`（被自动合并的字段列表）；有冲突则返回 `409 EDIT_CONFLICT`，错误体 `details` 形如：

```json
{
  "currentUpdatedAt": "2026-09-20T08:05:11.000Z",
  "conflicts": [
    { "field": "title", "base": "旧标题", "theirs": "对方改的标题", "yours": "我改的标题" },
    { "field": "attributes.shade", "base": "full", "theirs": "none", "yours": null }
  ],
  "merged": { "title": "我改的标题", "description": "已自动合并的描述" }
}
```

- 字段标识：`title` / `description` / `categoryCode` / `location`（经纬度整体）/ `fuzz`（模糊设置整体）/ `mediaUuids` / `attributes.<key>`；值为 `null` 表示该侧无此值（如属性被删除）；
- 编辑者逐项确认后，把选定值写回表单并带 `resolveConflicts: true` 与原 `base`、`baseUpdatedAt` 重提，冲突字段以提交值为准，其余字段仍按三路合并处理。
...
| A16 | 并发编辑按字段自动合并，冲突可逐项确认 | 两人同时编辑同一条目：改不同字段时双方改动都保留；改同一字段时后保存者收到 409 并在页面上逐项选择，确认后按选择生效 |
